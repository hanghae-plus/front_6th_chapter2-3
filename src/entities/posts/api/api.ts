...
const postsApi = {
   ...
   * 게시물 수정
   */
  updatePost: async (postData: Partial<Post>): Promise<Post> => {
    return api.put<Post, Partial<Post>>(`/posts/${postData.id}`, postData);
  },
   ...
   * 사용 가능한 태그 목록 조회
   */
  getTags: async (): Promise<PostTag[]> => {
    return api.get<PostTag[]>('/posts/tags');
  },
};
