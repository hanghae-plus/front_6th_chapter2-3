...
export const api = {
  get: <T>(path: string, opts: BaseOpts = {}) =>
    request<T>(`${path}${queryString(opts.params)}`, { method: "GET", headers: opts.headers }),
  ...
  delete: <T>(path: string, opts: BaseOpts = {}) => request<T>(path, { method: "DELETE", headers: opts.headers }),
}
